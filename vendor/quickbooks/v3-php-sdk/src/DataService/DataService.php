...
class DataService
{
    ...
    public function postInvoice()
    {
        ...
        //The CompanyInfo URL
        //$uri = implode(CoreConstants::SLASH_CHAR, array('company', $currentServiceContext->realmId, 'companyinfo', $currentServiceContext->realmId));
        $uri = "/v3/company/4620816365227335860/invoice?minorversion=65";
        ...
        $dummy = new \stdClass();
        $dummy->DetailType = "SalesItemLineDetail";
        $dummy->Amount = 100;
        $dummy->SalesItemLineDetail = $SalesItemLineDetail;
        $MainObj->Line = array($dummy);
        $ref = new \stdClass();
        $ref->value = "1";
        $MainObj->CustomerRef = $ref;
       
        $restRequestHandler = $this->getRestHandler();

        list($responseCode, $responseBody) = $restRequestHandler->sendRequest($requestParameters,json_encode($MainObj), null, $this->isThrownExceptionOnError());
